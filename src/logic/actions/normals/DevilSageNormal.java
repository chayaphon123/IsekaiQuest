...
public class DevilSageNormal extends NormalAction {
    ...
    public void activate(BaseCharacter targetCharacter) {
        int additionalDmg = 0;
        for(int i=0; i< GameController.getInstance().getAllCards().size(); i++) {
            BaseCharacter character = GameController.getInstance().getAllCards().get(i).getController().getCharacter();
